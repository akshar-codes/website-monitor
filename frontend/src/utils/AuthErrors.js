...
const GENERIC_FALLBACKS = {
  ...
  register: "Unable to create your account. Please try again.",
  verify: "Unable to verify your email. Please try again.",
  reset: "Unable to complete this request. Please try again.",
};

/**
 * Map an axios error from an auth request to a user-facing message.
 * `context` is one of "login" | "register" | "verify" | "reset" and only
 * affects the generic fallback used when the server didn't supply its own
 * message.
 */
export function mapAuthError(err, context = "login") {
  ...
  return serverMessage || GENERIC_FALLBACKS[context] || GENERIC_FALLBACKS.login;
}
